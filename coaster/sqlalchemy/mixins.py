...
from sqlalchemy.orm import Mapped, declarative_mixin, synonym
...
class IdMixin:
    ...
    @declared_attr
    def id(  # noqa: A003  # pylint: disable=no-self-argument
        cls,
    ) -> Mapped[t.Union[sa.Integer, uuid_.UUID]]:
    ...
    @declared_attr
    def url_id(  # pylint: disable=no-self-argument
        cls,
    ) -> Mapped[t.Union[int, uuid_.UUID]]:
...
class UuidMixin:
    ...
    @with_roles(read={'all'})
    @declared_attr
    def uuid(  # pylint: disable=no-self-argument
        cls,
    ) -> Mapped[uuid_.UUID]:
...
class TimestampMixin:
    ...
    @immutable
    @declared_attr
    def created_at(cls) -> sa.Column[sa.TIMESTAMP]:  # pylint: disable=no-self-argument
    ...
    @declared_attr
    def updated_at(cls) -> sa.Column[sa.TIMESTAMP]:  # pylint: disable=no-self-argument
...
class BaseNameMixin(BaseMixin):
    ...
    @declared_attr
    def name(cls) -> sa.Column[sa.Unicode]:  # pylint: disable=no-self-argument
    ...
    @declared_attr
    def title(cls) -> sa.Column[sa.Unicode]:  # pylint: disable=no-self-argument
    ...
    def make_name(self, reserved: t.Collection[str] = ()):
        ...
        """
        if self.title:
            if sa.inspect(self).has_identity:
...
class BaseScopedIdMixin(BaseMixin):
    ...
    @with_roles(read={'all'})
    @declared_attr
    def url_id(cls) -> sa.Column[sa.Integer]:  # pylint: disable=no-self-argument
